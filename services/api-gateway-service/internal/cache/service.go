...
// Service handles Redis cache operations for the API gateway
type Service struct {
	client  *redis.Client
	config  Config
...
func (s *Service) Get(key string) (string, error) {
	...
	s.metrics.RecordCacheLatency("get", latency)
	s.totalOps++
	...
	if err == redis.Nil {
		...
		s.missCount++
		s.updateHitRate()
		s.metrics.IncrementCacheRequests("get", "miss")
	...
	s.hitCount++
	s.updateHitRate()
	s.metrics.IncrementCacheRequests("get", "hit")
...
func (s *Service) Set(key, value string, ttl time.Duration) error {
	...
	latency := time.Since(startTime)
	s.metrics.RecordCacheLatency("set", latency)

	if err != nil {
		s.metrics.IncrementCacheErrors()
		s.errorCount++
		s.metrics.IncrementCacheRequests("set", "error")
		return fmt.Errorf("failed to set cache: %w", err)
	}

	s.metrics.IncrementCacheRequests("set", "success")
...
func (s *Service) Delete(key string) error {
	...
	if err != nil {
		s.metrics.IncrementCacheErrors()
		s.metrics.IncrementCacheRequests("delete", "error")
		return fmt.Errorf("failed to delete from cache: %w", err)
	}

	s.metrics.IncrementCacheRequests("delete", "success")
